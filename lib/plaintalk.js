...
"error", "crlf", "CR must be immediately followed by LF"
"error", "escape-invalid", "Invalid byte found within {}. Only decimal digits allowed"
"error", "escape-overflow", "I'm sure you did not want to escape more than 1000 bytes"
...
"readEscapeCount", this.escapeCount, this.escapeCountString
"ignored", data
*/
...
PlainTalk.prototype.readingEscapeCount = function (data) {
	...
	while (data[i] >= '0'.charCodeAt(0) && data[i] <= '9'.charCodeAt(0)) {
		...
		if (this.escapeCount > 1000) {
			...
			this.emit("ignored", Buffer.from("{" + this.escapeCountString));
			return data.subarray(i);
		}
	}
